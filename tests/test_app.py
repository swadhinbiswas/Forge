...
class TestForgeApp:
    ...
    def test_menu_api_manages_menu_state_and_emits_events(self, tmp_path: Path) -> None:
        config_path = _write_config(
            tmp_path, permissions="[permissions]\nmenu = true\n"
        )
        app = ForgeApp(str(config_path))
...
#         assert captured["ran"] is True
    ...
    def test_production_runtime_defaults_disable_devtools_and_uses_hardened_csp(self, tmp_path: Path, monkeypatch) -> None:
        monkeypatch.setenv("FORGE_ENV", "production")
        config_path = _write_config(tmp_path)
        app = ForgeApp(str(config_path))

        assert app.devtools_enabled() is False
        csp = app.content_security_policy()
        assert "'unsafe-eval'" not in csp
        assert "http://localhost:*" not in csp

    def test_security_allow_devtools_override_enables_devtools(self, tmp_path: Path, monkeypatch) -> None:
        monkeypatch.setenv("FORGE_ENV", "production")
        config_path = _write_config(
            tmp_path,
            permissions="[security]\nallow_devtools = true\n",
    ...
    def test_runtime_devtools_methods_reject_when_disabled(self, tmp_path: Path, monkeypatch) -> None:
        monkeypatch.setenv("FORGE_ENV", "production")
        config_path = _write_config(tmp_path)
        app = ForgeApp(str(config_path))
        app._proxy = self._FakeWindowProxy()
        ...
        try:
            ...
            raise AssertionError("Expected PermissionError when devtools are disabled")

    def test_task_ipc_start_returns_real_task_id(self, tmp_path: Path) -> None:
        config_path = _write_config(tmp_path)
        app = ForgeApp(str(config_path))

        started = json.loads(
            app.bridge.invoke_command(
                json.dumps({"command": "task_start", "args": {"name": "heartbeat", "interval": 0.01}, "id": 301})
            )
        )
        assert started["error"] is None
        task_id = started["result"]
        assert isinstance(task_id, str)
        status = app.tasks.status(task_id)
