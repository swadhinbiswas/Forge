...
def test_verify_alignment_reports_matching_versions(tmp_path: Path) -> None:
    ...
    (workspace / "pyproject.toml").write_text(
        "\n".join(["[project]", 'version = "3.0.0"']),
        encoding="utf-8",
    )
    (workspace / "package.json").write_text(json.dumps({"version": "3.0.0"}), encoding="utf-8")
    forge_pkg = workspace / "forge"
    forge_pkg.mkdir()
    (forge_pkg / "__init__.py").write_text('__version__ = "3.0.0"\n', encoding="utf-8")
...
def test_verify_alignment_reports_mismatches(tmp_path: Path) -> None:
    ...
    (workspace / "package.json").write_text(json.dumps({"version": "3.1.0"}), encoding="utf-8")
    forge_pkg = workspace / "forge"
    forge_pkg.mkdir()
    (forge_pkg / "__init__.py").write_text('__version__ = "3.0.0"\n', encoding="utf-8")
    forge_cli_pkg = workspace / "forge_cli"
    forge_cli_pkg.mkdir()
    (forge_cli_pkg / "__init__.py").write_text('__version__ = "3.1.0"\n', encoding="utf-8")
