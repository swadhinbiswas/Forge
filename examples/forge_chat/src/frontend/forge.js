...
(function () {
    ...
    function handleMessage(msg) {
        ...
        if (data.type === "reply" || ("id" in data && ("result" in data || "error" in data))) {
            ...
            // Python → JS channel stream
            var cbc = channelListeners.get(data.channel_id);
            if (cbc) {
                try {
                    cbc(data.data, data.done);
                } catch (e) {
                    console.error("[Forge] Channel listener error:", e);
                }
                if (data.done) {
                    channelListeners.delete(data.channel_id);
                }
            }
    ...
    window.__forge__ = {
        ...
        logger: {
            info: function(msg, ctx) { return window.__forge__.invoke("__forge_runtime_log_from_js", { level: "info", message: msg, context: ctx || null }); },
            warn: function(msg, ctx) { return window.__forge__.invoke("__forge_runtime_log_from_js", { level: "warn", message: msg, context: ctx || null }); },
            error: function(msg, ctx) { return window.__forge__.invoke("__forge_runtime_log_from_js", { level: "error", message: msg, context: ctx || null }); },
            debug: function(msg, ctx) { return window.__forge__.invoke("__forge_runtime_log_from_js", { level: "debug", message: msg, context: ctx || null }); }
        },

        tasks: {
            start: function (name, interval) {
                return window.__forge__.invoke("task_start", { name: name, interval: interval || null });
            },
            cancel: function (taskId) {
                return window.__forge__.invoke("task_cancel", { task_id: taskId });
            },
            status: function (taskId) {
                return window.__forge__.invoke("task_status", { task_id: taskId });
            },
            list: function () {
                return window.__forge__.invoke("task_list", {});
            }
        },

        channel: {
            on: function (channelId, callback) {
                channelListeners.set(channelId, callback);
            },
            off: function (channelId) {
                channelListeners.delete(channelId);
            }
        },

        ws: {
            connect: function (url, auto_reconnect) {
                return window.__forge__.invoke("ws_connect", { 
                    url: url, 
                    auto_reconnect: !!auto_reconnect 
                });
            },
            send: function (connectionId, data) {
                return window.__forge__.invoke("ws_send", { connection_id: connectionId, data: data });
            },
            sendBinary: function (connectionId, dataB64) {
                return window.__forge__.invoke("ws_send_binary", { connection_id: connectionId, data_b64: dataB64 });
            },
            close: function (connectionId) {
                return window.__forge__.invoke("ws_close", { connection_id: connectionId });
            },
            state: function (connectionId) {
                return window.__forge__.invoke("ws_state", { connection_id: connectionId });
            },
            list: function () {
                return window.__forge__.invoke("ws_list", {});
            }
        },

        http: {
            get: function (url) {
                return window.__forge__.invoke("http_get", { url: url });
            },
            postJson: function (url, body) {
                return window.__forge__.invoke("http_post_json", { url: url, json_body: body });
            },
            request: function (method, url, options) {
                options = options || {};
                return window.__forge__.invoke("http_request", {
                    method: method,
                    url: url,
            ...
            download: function (url, destination, options) {
                options = options || {};
                return window.__forge__.invoke("http_download", {
                    url: url,
                    destination: destination,
                    headers: options.headers || null
                });
            },
            upload: function (url, filePath, options) {
                options = options || {};
                return window.__forge__.invoke("http_upload", {
                    url: url,
                    file_path: filePath,
                    headers: options.headers || null,
                    field_name: options.field_name || "file"
                });
            }
        },

        opener: {
            openUrl: function (url) { return window.__forge__.invoke("open_url", { url: url }); },
            openPath: function (path) { return window.__forge__.invoke("open_path", { path: path }); },
            revealInFolder: function (path) { return window.__forge__.invoke("reveal_in_folder", { path: path }); }
        },

        positioner: {
            center: function (label) { return window.__forge__.invoke("position_center", { label: label || null }); },
            topRight: function (label, margin) { return window.__forge__.invoke("position_top_right", { label: label || null, margin: margin || 20 }); },
            bottomRight: function (label, margin) { return window.__forge__.invoke("position_bottom_right", { label: label || null, margin: margin || 20 }); },
            topLeft: function (label, margin) { return window.__forge__.invoke("position_top_left", { label: label || null, margin: margin || 20 }); },
            bottomLeft: function (label, margin) { return window.__forge__.invoke("position_bottom_left", { label: label || null, margin: margin || 20 }); },
            trayAnchor: function (label) { return window.__forge__.invoke("position_tray_anchor", { label: label || null }); }
        },

        window: {
            send: function (targetLabel, event, payload) {
                return window.__forge__.invoke("window_send", { 
                    target_label: targetLabel, 
                    event: event, 
                    payload: payload || null 
                });
            },
            broadcast: function (event, payload, excludeSelf) {
                return window.__forge__.invoke("window_broadcast", { 
                    event: event, 
                    payload: payload || null, 
         ...
         */
        _isNative: function () { return checkNativeIPC(); }
    };
    ...
    if (document.readyState === "complete" || document.readyState === "interactive") {
        setTimeout(initAutoConnect, 0);
    } else {
        document.addEventListener("DOMContentLoaded", initAutoConnect);
    }

    console.log("[Forge] Runtime v2.0 initialized.");
})();
