...
pub fn build_webview_for_window(
    ...
    webview_builder = webview_builder.with_asynchronous_custom_protocol("forge-asset".into(), move |_webview_id, request, responder| {
        let path_str = request.uri().path().to_string();
        
        // URL decode the path
        let decoded = urlencoding::decode(&path_str).unwrap_or(std::borrow::Cow::Borrowed(&path_str)).to_string();
        ...
        std::thread::spawn(move || {
            let is_allowed: Result<bool, pyo3::PyErr> = Python::attach(|py| {
                ...
                let allowed = scope_mod.call_method1("_validate_asset_path", (&file_path.to_string_lossy().to_string(),))?.extract::<bool>()?;
                Ok(allowed)
            });
            ...
            if !is_allowed.unwrap_or(false) {
                ...
                return responder.respond(builder.body(format!("Access Denied by
 Scope Validator: {}", decoded).into_bytes()).unwrap());                            }
    ...
    webview_builder = webview_builder.with_asynchronous_custom_protocol("forge-memory".into(), move |_webview_id, request, responder| {
        let path_str = request.uri().path().to_string();
        ...
        std::thread::spawn(move || {
            let result: Result<Vec<u8>, PyErr> = Python::attach(|py| {
                ...
                let memory_dict = forge_module.getattr("buffers")?.downcast_into::<pyo3::types::PyDict>()?;
                
                if let Some(py_bytes) = memory_dict.get_item(&key)? {
                    // Extract into bytes slice directly avoiding any other conversions
                    let extracted = py_bytes.downcast::<pyo3::types::PyBytes>()?;
                    ...
                    Err(pyo3::exceptions::PyKeyError::new_err(format!("Memory {} not found", key)))
                }
            });
            ...
            match result {
                ...
                Err(e) => {
                    eprintln!("[forge-core] forge-memory:// fetch failed for key {}: {:?}", key, e);
                    let response = wry::http::Response::builder()
                        .status(404)
